...
row_medians <- function(x) {
  #apply over each row of x
  return(apply(x, 1, median, na.rm=1))
...
summarize_rows <- function(x, fn, na.rm=FALSE) {
  # apply over each row 
  # na.rm is equal to the input na.rm of the function
...
summarize_matrix <- function(x, na.rm=FALSE) {
  # init summary df with mean
  summary_df = data.frame(apply(x, 1, mean, na.rm=na.rm))
  summary_df$stdev = apply(x, 1, sd, na.rm=na.rm)
  summary_df$median = apply(x, 1, median, na.rm=na.rm)
  summary_df$min = apply(x, 1, min, na.rm=na.rm)
  summary_df$max = apply(x, 1, max, na.rm=na.rm)
  # get the number of elements that are below zero by summing the rowwise calculations
  summary_df$num_lt_0 = apply(x, 1, function(x) {
    return(sum(less_than_zero(x)))
  })
  summary_df$num_btw_1_and_5 = apply(x, 1, function(x, a=1, b=5) {
    return(sum(is_between(x,a,b)))
  })
  summary_df$num_na = apply(x, 1, function(x) {
    return(length(x) - length(rm_na(x)))
  })
  colnames(summary_df)[1] = "mean"
  return(summary_df)
}
